...
max_cores<-as.integer(Sys.getenv("ICU_PIPELINE_MAX_CORES","4"))
cores<-min(max_cores,max(1L,as.integer(Sys.getenv("ICU_PIPELINE_CORES","3"))))
bootstrap_cores<-min(2L,cores,max(1L,as.integer(Sys.getenv("ICU_BOOTSTRAP_CORES","2"))))
append_log(master,"PARALLEL SAFETY main workers=",cores,"bootstrap workers=",bootstrap_cores,"BLAS threads=1")
