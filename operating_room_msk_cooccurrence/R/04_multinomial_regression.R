...
# R: 4.5.0
# Key packages: nnet, broom, data.table, openxlsx, ggplot2
...
for (pkg in c("nnet", "broom", "data.table", "openxlsx", "ggplot2")) {
...
regression_input_n <- nrow(analysis)
bmi_missing_or_nonfinite <- is.na(analysis$BMI5) | !is.finite(analysis$BMI5)
bmi_excluded_n <- sum(bmi_missing_or_nonfinite)
analysis <- droplevels(analysis[!bmi_missing_or_nonfinite, , drop = FALSE])
non_bmi_primary_predictors <- setdiff(primary_predictors, "BMI5")
if (any(!stats::complete.cases(analysis[c("class_outcome", non_bmi_primary_predictors)]))) {
  stop("Non-BMI primary-model variables contain missing values after BMI-missing records were excluded")
}
if (!nrow(analysis)) stop("No regression records remain after excluding missing or non-finite BMI")
model_datasets <- list(analysis)
regression_sample_audit <- data.frame(
  metric = c(
    "regression_input_rows",
    "bmi_missing_or_nonfinite_excluded_rows",
    "regression_complete_case_rows"
  ),
  value = c(regression_input_n, bmi_excluded_n, nrow(analysis)),
  stringsAsFactors = FALSE
)
cat(
  "Complete-case regression after BMI exclusion: input n =", regression_input_n,
  "; BMI missing/non-finite excluded n =", bmi_excluded_n,
  "; final n =", nrow(analysis), "\n"
)
...
primary_model_complete_n <- sum(stats::complete.cases(analysis[all.vars(primary_formula)]))
if (primary_model_complete_n != nrow(analysis)) {
  stop("Primary model would silently exclude records beyond the prespecified BMI exclusion")
}
regression_sample_audit <- rbind(
  regression_sample_audit,
  data.frame(metric = "primary_model_complete_rows", value = primary_model_complete_n, stringsAsFactors = FALSE)
)

primary_result <- fit_model_list(model_datasets, primary_formula)
primary_pooled <- pool_multinom(primary_result$tidy)
primary_pooled$model <- "Primary_complete_case_BMI_excluded"
...
univariable_any_night <- pool_multinom(fit_model_list(model_datasets, class_outcome ~ any_night_shift)$tidy)
univariable_any_night$model <- "Univariable_any_night_shift"
univariable_overtime <- pool_multinom(fit_model_list(model_datasets, class_outcome ~ overtime_weeks_last_month)$tidy)
univariable_overtime$model <- "Univariable_overtime"
univariable_pooled <- rbind(univariable_any_night, univariable_overtime)

work_year_result <- fit_model_list(model_datasets, work_year_formula)
work_year_pooled <- pool_multinom(work_year_result$tidy)
work_year_pooled$model <- "Sensitivity_work_years_adjusted_complete_case"

night_subset <- function(dataset) dataset[dataset$any_night_shift == "Yes", , drop = FALSE]
night_result <- fit_model_list(model_datasets, night_formula, subset_function = night_subset)
...
posterior_datasets <- rep(list(analysis), config$runtime$posterior_draws)
...
pseudo_result <- fit_model_list(posterior_datasets, primary_formula, outcome_override = draw_outcome)
...
vif_primary <- calculate_vif(model_datasets[[1]], linear_primary_formula)
vif_night <- calculate_vif(night_subset(model_datasets[[1]]), linear_night_formula)

linearity_complete <- analysis
...
model_fit_rows <- do.call(rbind, lapply(seq_along(primary_result$fits), function(index) {
  ...
  null_fit <- nnet::multinom(class_outcome ~ 1, data = model_datasets[[index]], trace = FALSE)
  predicted <- predict(fit, type = "class")
  data.frame(
    analysis_dataset = index,
    n = nrow(model_datasets[[index]]),
    log_likelihood = as.numeric(stats::logLik(fit)),
    AIC = stats::AIC(fit),
    McFadden_R2 = 1 - as.numeric(stats::logLik(fit)) / as.numeric(stats::logLik(null_fit)),
    modal_accuracy = mean(predicted == model_datasets[[index]]$class_outcome)
  )
}))
...
model_matrix_columns <- ncol(stats::model.matrix(primary_formula, data = model_datasets[[1]])) - 1
...
marginal_contrast <- function(fits, datasets, variable, low_value, high_value, label) {
  ...
  for (index in seq_along(fits)) {
    ...
    rows[[index]] <- data.frame(analysis_dataset = index, class = names(low_probability), risk_low = low_probability, risk_high = high_probability, risk_difference = high_probability - low_probability)
  }
...
marginal_any_night <- marginal_contrast(primary_result$fits, model_datasets, "any_night_shift", "No", "Yes", "Any night shift: Yes vs No")
marginal_overtime <- marginal_contrast(primary_result$fits, model_datasets, "overtime_weeks_last_month", 0, 4, "Weeks >40 h: 4 vs 0")
marginal_effects <- rbind(marginal_any_night, marginal_overtime)

data.table::fwrite(primary_pooled, file.path(output_dir, "multinomial_primary_complete_case.csv"), bom = TRUE)
...
data.table::fwrite(model_fit_rows, file.path(output_dir, "multinomial_model_fit_primary.csv"), bom = TRUE)
...
data.table::fwrite(regression_sample_audit, file.path(output_dir, "regression_complete_case_audit.csv"), bom = TRUE)

workbook <- openxlsx::createWorkbook()
sheet_data <- list(
  primary_complete_case = primary_pooled,
  ...
  regression_sample_audit = regression_sample_audit
)
...
cat("\nPrimary exposure results (complete-case regression after BMI exclusion):\n")
