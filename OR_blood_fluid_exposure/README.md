...
需要：`data.table`、`psych`、`sandwich`、`lmtest`、`car`、`quantreg`、`MatchIt`、`ggplot2`。仅在提供 `OR_DICTIONARY_FILE` 时才需要 `readxl`；使用已经载入的 `data` 且不读取量表文件时不需要安装 `readxl`。重叠加权模型使用 `sandwich` 稳健标准误，不依赖 `survey`。
